...
async function bootstrap() {
  ...
  try {
    let app;
    if (
      process.env.STAGE.toLowerCase() === 'local' ||
      process.env.USE_PROTOCOL === 'http'
    ) {
    ...
    if (
      ...
      process.env.WHITELIST_ORIGIN.split(',').length > 0
    ) {
      const whitelist = process.env.WHITELIST_ORIGIN.split(',');
      app.enableCors({
